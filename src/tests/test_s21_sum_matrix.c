...
START_TEST(positive_test) {
  ...
  expected.matrix[0][1] = 8.0;
  expected.matrix[1][0] = 10.0;
  expected.matrix[1][1] = 12.0;

  int codeError = s21_sum_matrix(&A, &B, &actual);
...
Suite *suiteSumMatrix(void) {
  Suite *s = suite_create("suite_sumMatrix");
  TCase *tc = tcase_create("tc_sumMatrix");
